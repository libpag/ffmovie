    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        case AVMEDIA_TYPE_VIDEO:
          for (int j = 0; j < st->nb_index_entries; j++) {
            st->index_entries[j].pts = st->index_entries[j].timestamp;
            if (sc->ctts_data && j < sc->ctts_count) {
              st->index_entries[j].pts = st->index_entries[j].timestamp + sc->dts_shift + sc->ctts_data[j].duration;
            }
          }
            if (sc->display_matrix) {
...
static int index_search_pts(const AVIndexEntry *entries, int nb_entries,
                              int64_t wanted_pts)
{
    if (nb_entries == 0) {
        return nb_entries;
    }
    int start = 0, end = nb_entries - 1;
    if (wanted_pts >= entries[end].pts) {
        return end;
    }
    while (start <= end) {
        int mid = (start + end) /2;
        if (entries[mid].pts <= wanted_pts) {
            start = mid + 1;
        } else if (entries[mid].pts > wanted_pts) {
            end = mid -1;
        }
    }
    if (start == 0) {
...
static int mov_seek_stream(AVFormatContext *s, AVStream *st, int64_t timestamp, int flags)
{
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && st->index_entries && st->nb_index_entries > 0) {
        int index = index_search_pts(st->index_entries, st->nb_index_entries, timestamp);
