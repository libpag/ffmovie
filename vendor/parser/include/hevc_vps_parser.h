#ifndef PAG_HEVC_VPS_PARSER_H
#define PAG_HEVC_VPS_PARSER_H
...
static bool hevc_parse_inner_profile_tier_level_rbsp(bs_t* p_bs,
  ...
  if (bs_remain(p_bs) < 88) return false;
  ...
  if ((p_in->profile_idc >= 4 && p_in->profile_idc <= 10) ||
    ...
    if (p_in->profile_idc == 5 || p_in->profile_idc == 9 || p_in->profile_idc == 10 ||
        (p_in->profile_compatibility_flag & 0x08600000)) {
      p_in->idc4to7.max_14bit_constraint_flag = bs_read1(p_bs);
      bs_skip(p_bs, 33);
    } else
      bs_skip(p_bs, 34);
  } else if (p_in->profile_idc == 2 || (p_in->profile_compatibility_flag & 0x20000000)) {
    bs_skip(p_bs, 7);
    p_in->idc4to7.one_picture_only_constraint_flag = bs_read1(p_bs);
    bs_skip(p_bs, 35);
  } else {
  ...
  if ((p_in->profile_idc >= 1 && p_in->profile_idc <= 5) || p_in->profile_idc == 9 ||
...
static bool hevc_parse_profile_tier_level_rbsp(bs_t* p_bs, bool profile_present,
  ...
  if (bs_remain(p_bs) < 8) return false;

  p_ptl->general_level_idc = bs_read(p_bs, 8);

  if (max_num_sub_layers_minus1 > 0) {
    if (bs_remain(p_bs) < 16) return false;

    for (uint8_t i = 0; i < 8; i++) {
      if (i < max_num_sub_layers_minus1) {
        if (bs_read1(p_bs)) p_ptl->sublayer_profile_present_flag |= (0x80 >> i);
        if (bs_read1(p_bs)) p_ptl->sublayer_level_present_flag |= (0x80 >> i);
      } else
        bs_read(p_bs, 2);
    }
...
static bool hevc_parse_video_parameter_set_rbsp(bs_t* p_bs, hevc_video_parameter_set_t* p_vps) {
  if (bs_remain(p_bs) < 134) return false;
  ...
  for (unsigned i =
           (p_vps->vps_sub_layer_ordering_info_present_flag ? 0 : p_vps->vps_max_sub_layers_minus1);
  ...
  if (bs_remain(p_bs) < 10) return false;
  ...
  if (bs_remain(p_bs) < 2) return false;
  ...
  if (bs_remain(p_bs) < 1) return false;
